...
def get_closest(vector, database):
    ...
    # print(f'Min distance: {distances[sorted_idx[0]]}')
    if distances[sorted_idx[0]] > 1.0:
        name = "Unknown"
    else:
...
def process_frame(img_pil: Union[Image.Image, np.ndarray], db):
    ...
    results = yolo.predict(img_pil, iou=0.8, conf=0.3,
                           imgsz=640, device=device, stream=True)
    img_tensor = F.to_tensor(img_pil).unsqueeze(0)
    for box in next(results).boxes.xyxy:
        box = box.int()
...
output_path = os.path.join(cwd, "live_feed_out")
i = 0
database = load_encodings(embd_dir)
url = "http://192.168.0.205:4747/video"
cap = cv2.VideoCapture(0)
while True:
    success, frame = cap.read()
    if not success:
        break
    output = process_frame(frame, database)
    img = annotate_image(frame, output)
    # img = cv2.flip(img, 1)
    cv2.imshow("Output", img)
    cv2.imwrite(os.path.join(output_path, f"frame_{i}.png"), img)
    i += 1
